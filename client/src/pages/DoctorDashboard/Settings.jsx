...
import DoctorTwoFactorAuth from './TwoFactorAuth';

const DoctorSettings = ({ doctorData }) => {
  ...
  useEffect(() => {
    localStorage.setItem('doctorDevices', JSON.stringify(devices));
  }, [devices]);
  ...
  const fetchTwoFactorStatus = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/api/doctors/2fa/status/${doctorData?._id || ''}`
      );
  ...
  const handlePasswordChange = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:5000/api/doctors/change-password/${doctorData._id}`, {
        method: 'PUT',
  ...
  return (
    <div className="dashboard-home" style={containerStyle}>
      ...
      {/* Notifications Settings (SMS removed for doctors) */}
      <div style={{ ...sectionStyle, borderTop: '4px solid #10b981' }}>
        <h3 style={h3Style}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <p style={{ color: '#6b7280', fontSize: '0.875rem', marginLeft: 32 }}>Receive appointment confirmations and updates via email</p>
        </div>
        <div style={{ marginBottom: 16 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', marginBottom: 8 }}>
            <input 
              ...
              checked
            />
            <span style={{ fontWeight: 500, color: '#374151' }}>Appointment Reminders</span>
          </label>
          <p style={{ color: '#6b7280', fontSize: '0.875rem', marginLeft: 32 }}>Get notified before upcoming appointments</p>
        </div>
      </div>
