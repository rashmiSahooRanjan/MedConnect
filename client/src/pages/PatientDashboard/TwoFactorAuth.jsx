...
const TwoFactorAuth = ({ patientData, isOpen, onClose, onStatusChange }) => {
  ...
  const fetchTwoFactorStatus = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/patients/2fa/status/${patientData._id}`);
  ...
  const handleSetup2FA = async () => {
    try {
      ...
      setTwoFactorError('');
      const response = await fetch(`http://localhost:5000/api/patients/2fa/setup/${patientData._id}`, {
        method: 'POST'
      });
      const data = await response.json();
      if (data.success) {
  ...
  const handleVerifyAndEnable2FA = async () => {
    ...
    try {
      setTwoFactorLoading(true);
      setTwoFactorError('');
      const response = await fetch(`http://localhost:5000/api/patients/2fa/enable/${patientData._id}`, {
        method: 'POST',
      ...
      if (data.success) {
        ...
        await fetch(`http://localhost:5000/api/communication/sms/2fa-enabled/${patientData._id}`, { method: 'POST' });
      } else {
  ...
  const handleDisable2FA = async () => {
    ...
    try {
      setTwoFactorLoading(true);
      setTwoFactorError('');
      const response = await fetch(`http://localhost:5000/api/patients/2fa/disable/${patientData._id}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.success) {
        ...
        onStatusChange && onStatusChange();
        // Send SMS notification
        await fetch(`http://localhost:5000/api/communication/sms/2fa-disabled/${patientData._id}`, { method: 'POST' });
      } else {
  ...
  const downloadBackupCodes = () => {
    ...
    a.href = url;
    a.download = 'medi-connect-backup-codes.txt';
    a.click();
  ...
  return (
    <div className="modal-overlay" onClick={closeModal} style={{
      ...
      <div className="two-factor-modal" onClick={(e) => e.stopPropagation()} style={{
        background: 'white',
        borderRadius: '16px',
        width: '90%',
        maxWidth: '500px',
            ...
            {qrCode && (
              <div style={{ marginBottom: '24px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px', marginTop: '12px', justifyContent: 'center' }}>
                  <input
                    readOnly
                    value={qrCode ? 'Secret copied to clipboard' : ''}
                    style={{
                      flex: 1,
                      padding: '8px 12px',
                      border: '1px solid #d1d5db',
