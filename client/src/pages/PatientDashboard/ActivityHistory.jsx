...
import { Calendar, User, FileText, CreditCard, Pill, Activity, Clock, CheckCircle, XCircle, ArrowLeftRight, Download } from 'lucide-react';

const ActivityHistory = ({ patientData }) => {
  ...
  useEffect(() => {
    if (patientData && patientData._id) {
      ...
      console.log('No patientData._id available:', patientData);
    }
  }, [patientData]);

  useEffect(() => {
    console.log('ActivityHistory mounted, patientData:', patientData);
  }, []);

  const fetchActivityHistory = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:5000/api/patients/history/${patientData._id}`);
      const data = await response.json();
      
      if (response.ok) {
        let historyData = [];
        if (data.success && data.activityHistory) {
          historyData = data.activityHistory;
        ...
        console.log('Loaded history:', sortedHistory.length, 'items');
      } else {
        console.error('Failed to fetch history:', data?.message || response.statusText);
      }
    } catch (error) {
  ...
  const tabs = [
    ...
    { id: 'profile', label: 'Profile', count: history.filter(h => h.category === 'profile').length }
  ];
    ...
    return (
      <div className="dashboard-home">
        <div className="history-container">
          <div className="loading-container">Loading activity history...</div>
        </div>
      </div>
    );
  }
          ...
          <div className="history-timeline">
            {Object.entries(groupedHistory).map(([date, items]) => (
              <div key={date} className="history-date-group">
                ...
                {items.map((item) => (
                  <div key={item._id || item.relatedId} className="history-item">
                    ...
                    <div className="history-content">
                      ...
                      {expandedItems[item._id || item.relatedId] && (
